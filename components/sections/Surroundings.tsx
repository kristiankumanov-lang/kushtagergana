    ...
    <section id="surroundings" className="scroll-mt-20 bg-cream py-20 sm:py-28">
      <Container>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2">
          <div className="rounded-2xl bg-pine-900 p-7 text-cream">
            <div className="flex items-center gap-3">
              <Snowflake className="size-6 text-pine-300" />
              <h3 className="font-display text-2xl">{t.surroundings.winter.title}</h3>
            </div>
            <ul className="mt-5 flex flex-col gap-3">
              {t.surroundings.winter.items.map((item) => (
                <li key={item} className="flex gap-2.5 text-sm text-cream/85 sm:text-base">
                  <span className="mt-2 size-1.5 shrink-0 rounded-full bg-pine-300" />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-2xl bg-gold-700 p-7 text-ink">
            <div className="flex items-center gap-3">
              <Sun className="size-6 text-ink" />
              <h3 className="font-display text-2xl text-ink">{t.surroundings.summer.title}</h3>
            </div>
            <ul className="mt-5 flex flex-col gap-3">
              {t.surroundings.summer.items.map((item) => (
                <li key={item} className="flex gap-2.5 text-sm text-ink sm:text-base">
